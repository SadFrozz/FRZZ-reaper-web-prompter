...
7. [Настройки и поведение](#настройки-и-поведение)
   ...
   4. [Сегментация проекта](#сегментация-проекта)
...
- Автосегментация по маркерам/видео и ручной режим разбиения проекта на блоки с собственным генератором длительностей.
...
### Сегментация проекта
- **Автоматический режим**: backend анализирует маркеры и/или видеотреки проекта (в зависимости от того, какие галочки включены в настройках). Приоритет между маркерами и видео задаётся в настройках, вручную настроенные сегменты всегда имеют наивысший приоритет. Все переключения отражаются в модалке статистики, где можно выбрать сегмент для отчёта. Если вдруг вы разрезали видео на части, то это также будет учтено при условии что имя файла одинаковое, а расстояние между видеосегментами менее секунды.
- **Ручной режим**: активируется отдельным тогглом в настройках. После включения появляется поле ввода длительности сегмента по умолчанию (в минутах) и раздел «Ручное сегментирование» с таблицей текущих блоков в модальном окне настроек проекта. Длительность по умолчанию используется генератором при создании новых записей и синхронно обновляет значения в форме, если пользователь их не редактировал вручную.
- **Генератор**: позволяет сгенерировать пачку сегментов поверх таймлайна проекта, задать маску имён и выбрать режим (добавить к текущим/заменить текущие). Из формы можно добавлять, редактировать и удалять строки, пока они не применены к проекту.
- **Статистика и навигация**: при активном ручном режиме все виджеты статистики и блок навигации сегментов автоматически переключаются на ручной источник. Маркеры и видео остаются доступными после отключения ручного режима.
...
**Q:** В проекте отображаются не все субтитры, а только те, что относятся к текущему видеофайлу/маркеру/региону/ручному сегменту.  
**A:** Проверьте настройки сегментации, некоторые настройки направлены на улучшение быстродействия при больших объёмах текстов либо более лучшего визуального восприятия. Если вам нужно видеть весь проект, поменяйте режим отображения сегментов на `Весь проект`.
