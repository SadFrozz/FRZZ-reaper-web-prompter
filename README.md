...
- Нет пока импорта форматов субтитров (.srt, .lrc) — в планах.
...
- Поддержка импорта форматов (srt / lrc)
...
**Q:** Не прикрепились инсталлеры в релизе.  
**A:** В версиях после v1.4.0 workflow прикрепляет артефакты автоматически.

**Q:** Где хранятся настройки?  
**A:** Persist через ExtState (chunked JSON) + файлы ролей (`<project>-roles.json`).
